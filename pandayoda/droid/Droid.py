...
class Droid(threading.Thread):
    ...
    def setup(self, job):
        try:
            ...
            if status != 0:
                logger.warning("Rank %s: failed to start stager thread(status: %s, output: %s)" % (self.__rank, status, output))
                return False, output
    ...
    def startStagerThread(self, job):
        ...
        try:
            ...
            logger.warning("Rank %s: Failed to initStagerThread: %s" % (self.__rank, str(traceback.format_exc())))
    ...
    def updateEventRange(self, output):
        try:
            ...
            logger.warning("Rank %s: failed to get eventRangeID from output: %s" % (self.__rank, output))
            logger.warning("Rank %s: error message: %s" % (self.__rank, str(e)))
    ...
    def run(self):
        ...
        while not self.__stop:
            ...
            try:
                ...
                logger.warning("Rank %s: Droid throws exception when running one job: %s" % (self.__rank, traceback.format_exc()))
                break
            os.chdir(self.__globalWorkingDir)
